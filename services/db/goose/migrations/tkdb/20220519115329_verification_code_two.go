...
func upVerificationCodeTwo(tx *sql.Tx) error {
	...
	for topRows.Next() {
		...
		topContainerIDs = append(topContainerIDs, cid)
	}

	for _, cid := range topContainerIDs {
		...
		if err := collectionGraph.TraverseUniqueEdges(func(id int64) error {
			...
			if err == sql.ErrNoRows {
				...
				return errors.Wrapf(err, "error selecting file_collection %d's children", id)
			}
			defer rows.Close()
