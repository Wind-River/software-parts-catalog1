...
func NewPartGraph(db *sql.Tx, partID uuid.UUID) (*PartGraph, error) {
	...
	rows, err := db.Query("SELECT child_id FROM part_has_part WHERE parent_id=$1", partID)
	if err != nil {
	...
	if len(pgraph.Edges) > 0 {
		if err := pgraph.Graph.TraverseUniqueEdges(func(id string) error {
			...
			rows, err := db.Query("SELECT child_id FROM part_has_part WHERE parent_id=$1", id) // TODO do I need to parse the uuid string for this query?
			if err != nil {
