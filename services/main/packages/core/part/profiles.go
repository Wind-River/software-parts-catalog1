...
func (controller PartController) GetProfile(partID ID, profile string) (*Profile, error) {
	...
	if err := controller.DB.QueryRowx("SELECT document FROM part_has_document WHERE part_id=$1 AND key=$2", partID, profile).Scan(&tmpDocument); err == sql.ErrNoRows {
	...
	rows, err := controller.DB.Queryx(`SELECT title, document FROM part_documents WHERE part_id=$1 AND key=$2`, partID, profile)
...
func (controller PartController) GetProfiles(partID ID) ([]Profile, error) {
	...
	rows, err := controller.DB.Queryx("SELECT key, document FROM part_has_document WHERE part_id=$1", partID)
	...
	rows, err = controller.DB.Queryx("SELECT key, title, document FROM part_documents WHERE part_id=$1", partID)
	if err != nil && err != sql.ErrNoRows {
		return nil, errors.Wrapf(err, "error selecting documents")
	} else if err == nil {
		defer rows.Close()
