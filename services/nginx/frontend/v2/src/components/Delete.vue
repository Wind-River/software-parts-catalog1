    ...
    methods: {
        ...
        search () {
            ...
            if (!isNaN(this.archiveID) && this.archiveID > 0) {
                ...
                this.loading = true
                var ok = true
                fetch(new Request('/api/archive?query='+encodeURIComponent(this.archiveID), { method: 'GET', mode: 'same-origin' }))
                .then((response) => {
                    ok = response.ok
                    this.loading = false
                    if (ok) {
        ...
        update () {
            ...
            if (payload.LicenseExpression != null || payload.LicenseRationale != null) { // make sure there is ANYTHING to be updated
                var ok = false
                fetch(new Request('/api/archive'), { method: 'POST', body: JSON.stringify(payload), mode: 'same-origin', headers: { 'Content-Type': 'application/json' }})
                .then(response => {
                    ok = response.ok
                    if (ok) {
                        this.search()
        ...
        del () {
            var ok = true
            fetch(new Request('/api/archive/'+this.archiveID), { method: 'DELETE', mode: 'same-origin'})
            .then(response => {
                ok = response.ok

                if (ok) {
                    let tmp = this.archiveID
                    this.reset()
                    this.deleted = tmp
                } else {
                    return response.text()
