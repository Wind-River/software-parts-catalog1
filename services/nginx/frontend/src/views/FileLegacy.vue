...
export default {
    ...
    uploadElements () {
      return this.uploadList.reduce((array, element) => {
        if (!element.progress.uploadComplete) {
          const perc = (element.progress.percentage * 100).toFixed(0)
          element.perc = perc
          array.push(element)
        }
  ...
  mounted () {
    ...
    this.uppy.on('file-added', (file) => {
      const ind = vm.uploadList.push(file) - 1
    ...
    this.uppy.on('upload-success', (file, payload, uploadURL) => {
      if (payload.isMeta === false) {
        ...
        if (Object.prototype.hasOwnProperty.call(payload, 'Extra')) {
    ...
    this.uppy.on('upload-progress', (file, progress) => {
      const f = vm.uploadList[vm.uploadDict[file.id]]
